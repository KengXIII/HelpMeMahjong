                            ...
                            if not isinstance(copyOfTiles[j], SuitedTile) or copyOfTiles[j].isInsideWinningHand:
                                ...
                                for k in range(j + 1, len(copyOfTiles)):
                                    if copyOfTiles[k].isInsideWinningHand:
                                        ...
                                        break
                count = 0
                for tile in copyOfTiles:
                    if tile.isInsideWinningHand:
                        count += 1
...
def main():
    ...
    hand = Hand(tiles)
    hand.printAll()
    hand.printInCategories()

    # print(hand.isWinningHand())
    for x in hand.isOneTileFromWinning():
        print(x)
